    ...
    jsonrpsee_types::error::ErrorObject<'static>: From<T::Error>,
{
    ...
    async fn block_receipts_trace(
        ...
        let block_rewards_task = tokio::spawn({
            ...
            async move {
                let maybe_block =
                    self_clone.recovered_block(BlockId::number(number)).await.unwrap();
                let mut trace_rewards: Vec<LocalizedTransactionTrace> = Vec::new();

                if let Some(block) = maybe_block {
                    if let Ok(Some(base_block_reward)) =
                        self_clone.calculate_base_block_reward(block.header())
                    {
        ...
        if let BlockTransactions::Full(transactions) = block.transactions {
            ...
            for ((trx, receipt), trace) in trx_iter.zip(receipts_iter).zip(trace_iter) {
                if trx.tx_hash() != trace.transaction_hash ||
                    trx.tx_hash() != receipt.transaction_hash()
                {
                ...
                {
                    alloy_trx =
                        serde_json::from_str::<op_alloy_rpc_types::Transaction>(&json_trx).unwrap();
                    alloy_receipt =
                        serde_json::from_str::<op_alloy_rpc_types::OpTransactionReceipt>(
                            &json_receipt,
                        )
                            .unwrap();
                }
                #[cfg(not(feature = "optimism"))]
                {
                    alloy_trx = serde_json::from_str::<alloy_rpc_types_eth::Transaction>(&json_trx)
                        .unwrap();
                    alloy_receipt =
                        serde_json::from_str::<alloy_rpc_types_eth::TransactionReceipt>(
                            &json_receipt,
                        )
                            .unwrap();
                }
                ...
                #[cfg(feature = "optimism")]
                {
                    is_bridge = matches!(alloy_trx.inner.inner.inner(), Deposit(_));
                }
